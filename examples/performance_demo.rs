...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: {} <fasta_file> [num_threads] [num_reads_per_thread]", args[0]);
    ...
    println!("Performance demonstration with {} threads, {} reads per thread", 
...
fn multi_threaded_test(index: FastaIndex, sequence_names: &[String], num_threads: usize, reads_per_thread: usize) {
    ...
    for (thread_id, handle) in handles.into_iter().enumerate() {
        match handle.join() {
            Ok((successful, bases)) => {
                ...
                total_bases += bases;
                println!("Thread {}: {}/{} successful reads, {} bases", 
    ...
    println!("Total successful reads: {}/{}", total_successful, num_threads * reads_per_thread);
    println!("Total bases read: {}", total_bases);
}
        ...
        let handle = thread::spawn(move || {
            ...
            for seq_name in &sequence_names_clone {
                if let Some(seq_len) = index_clone.sequence_length(seq_name) {
                    ...
                    let start_pos = (thread_id as i64 * region_size) % (seq_len - region_size).max(1);
                    let end_pos = start_pos + region_size;
                    
                    match reader.fetch_seq(seq_name, start_pos, end_pos) {
